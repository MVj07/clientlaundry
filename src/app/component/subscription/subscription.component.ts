...
export class SubscriptionComponent implements OnInit {
  ...
  subscribeNow(): void {
    ...
    this.subscriptionService.createOrder().subscribe({
      next: (orderRes) => {
        ...
        if (orderRes.mockMode) {
          const confirmSandbox = window.confirm(
            "[Sandbox Development Mode]\n\nNo live Razorpay keys (RAZORPAY_KEY_ID) detected in your .env file.\n\nClick OK to simulate a successful ₹150 payment verification and activate your 30-day Pro plan right now!"
          );
          if (confirmSandbox) {
            this.verifyPayment({
        ...
        const options = {
          ...
          description: 'Monthly All-Access Subscription (₹150/month)',
